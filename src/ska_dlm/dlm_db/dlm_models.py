...
class Location(Base):
    ...
    location_date: Mapped[Optional[datetime.datetime]] = mapped_column(
...
class PhaseChange(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint("phase_change_id", name="phase_change_pkey"),)

    phase_change_id: Mapped[int] = mapped_column(
        BigInteger,
        Identity(
            always=True,
            start=1,
    ...
    requested_phase: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'GAS'::character varying")
    )
    request_creation: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, server_default=text("now()")
    )
...
class Storage(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["location_id"], ["location.location_id"], ondelete="SET NULL", name="fk_location"
        ),
    ...
    storage_phase: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'GAS'::character varying")
    )
    storage_capacity: Mapped[Optional[int]] = mapped_column(
        BigInteger, server_default=text("'-1'::integer")
    )
    storage_use_pct: Mapped[Optional[decimal.Decimal]] = mapped_column(
        Numeric(3, 1), server_default=text("0.0")
    )
    storage_permissions: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'RW'::character varying")
    )
    ...
    storage_num_objects: Mapped[Optional[int]] = mapped_column(
    ...
    storage_date: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, server_default=text("now()")
    )
    ...
    migration: Mapped[List["Migration"]] = relationship(
        "Migration",
        foreign_keys="[Migration.destination_storage_id]",
        back_populates="destination_storage",
    )
    migration_: Mapped[List["Migration"]] = relationship(
        "Migration", foreign_keys="[Migration.source_storage_id]", back_populates="source_storage"
    )
    storage_config: Mapped[List["StorageConfig"]] = relationship(
        "StorageConfig", back_populates="storage"
    )
...
class DataItem(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["storage_id"], ["storage.storage_id"], ondelete="SET NULL", name="fk_storage"
        ),
    ...
    uri: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'inline://item_value'::character varying")
    )
    item_value: Mapped[Optional[str]] = mapped_column(Text, server_default=text("''::text"))
    item_type: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'file'::character varying")
    )
    item_format: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'unknown'::character varying")
    )
    item_encoding: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'unknown'::character varying")
    )
    item_mime_type: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'application/octet-stream'::character varying")
    )
    item_level: Mapped[Optional[int]] = mapped_column(
        SmallInteger, server_default=text("'-1'::integer")
    )
    uid_phase: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'GAS'::character varying")
    )
    oid_phase: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'GAS'::character varying")
    )
    item_state: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'INITIALIZED'::character varying")
    )
    uid_creation: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, server_default=text("now()")
    )
    oid_creation: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    uid_expiration: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, server_default=text("(now() + ('24:00:00'::time without time zone)::interval)")
    )
    oid_expiration: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, server_default=text("'2099-12-31 23:59:59'::timestamp without time zone")
    )
    ...
    checksum_method: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'none'::character varying")
    )
    last_check: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    item_owner: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'SKA'::character varying")
    )
    item_group: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'SKA'::character varying")
    )
...
class Migration(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["storage.storage_id"],
            ondelete="SET NULL",
            name="fk_destination_storage",
        ),
        ForeignKeyConstraint(
            ["source_storage_id"],
            ["storage.storage_id"],
            ondelete="SET NULL",
            name="fk_source_storage",
        ),
    ...
    destination_storage: Mapped["Storage"] = relationship(
        "Storage", foreign_keys=[destination_storage_id], back_populates="migration"
    )
    source_storage: Mapped["Storage"] = relationship(
        "Storage", foreign_keys=[source_storage_id], back_populates="migration_"
    )
...
class StorageConfig(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["storage_id"], ["storage.storage_id"], ondelete="SET NULL", name="fk_cfg_storage_id"
        ),
    ...
    config_type: Mapped[Optional[str]] = mapped_column(
        String, server_default=text("'rclone'::character varying")
    )
    config_date: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime, server_default=text("now()")
    )
