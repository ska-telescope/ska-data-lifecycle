...
def query_data_item(item_name: str = "", oid: str = "", uid: str = "", params: str = None) -> list:
    ...
    response = requests.get(f"{REQUEST_URL}/request/query_data_item", params=params, timeout=60)
...
def query_expired(offset: timedelta = None):
    ...
    """
    params = {k: v for k, v in locals().items() if v}
    response = requests.get(f"{REQUEST_URL}/request/query_expired", params=params, timeout=60)
    return response.json()
...
def query_deleted(uid: str = "") -> list:
    ...
    """
    params = {k: v for k, v in locals().items() if v}
    response = requests.get(f"{REQUEST_URL}/request/query_deleted", params=params, timeout=60)
    return response.json()
...
def query_new(check_date: str, uid: str = "") -> list:
    ...
    """
    params = {k: v for k, v in locals().items() if v}
    response = requests.get(f"{REQUEST_URL}/request/query_new", params=params, timeout=60)
    return response.json()
...
def query_exists(item_name: str = "", oid: str = "", uid: str = "", ready: bool = False) -> bool:
    ...
    """
    params = {k: v for k, v in locals().items() if v}
    response = requests.get(f"{REQUEST_URL}/request/query_exists", params=params, timeout=60)
    return response.json()
...
def query_item_storage(item_name: str = "", oid: str = "", uid: str = "") -> str:
    ...
    """
    params = {k: v for k, v in locals().items() if v}
    response = requests.get(f"{REQUEST_URL}/request/query_item_storage", params=params, timeout=60)
    return response.json()
